<template>
  <section class="generate-image">
    <div class="columns">
      <div class="column image-options">
      ...
      <div class="column is-three-quarters image-container" id="imageContainer">
        <article
          class="box is-unselectable output-image-html"
          ref="imageHtml"
          :style="imageBoxStyle"
        >
          <h3 class="title is-3 is-spaced" :style="imageTitleStyle">{{ renderText }}</h3>
          <h6 class="subtitle is-6" :style="imageAuthorStyle">{{ image.author }}</h6>
        </article>
        <button
...
@Component({
  ...
  methods: {
    ...
    handleBgImageChange(): void {
      const image = this.$refs.bgImage.files[0];
